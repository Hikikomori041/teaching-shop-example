...
export default function DailyRevenueChart({ orders }: Props) {
    ...
    const totalRevenue = useMemo(() => dailyRevenue.reduce((sum, d) => sum + d.revenue, 0), [dailyRevenue])
                    ...
                    return (
                        <div key={day.date} className="flex-1 flex flex-col items-center min-w-0">
                            <div className="w-full flex flex-col items-center justify-end h-40">
                                <span className="text-xs text-gray-600 mb-1 hidden sm:block">{formatRevenue(day.revenue)}</span>
                                <div className="w-full bg-primary hover:bg-primary-hover rounded-t transition-colors cursor-default" style={{ height: `${Math.max(heightPercent, 2)}%` }} title={`${day.displayDate}: $${day.revenue.toFixed(2)}`} />
                            </div>
                            <span className="text-xs text-gray-500 mt-2 truncate w-full text-center">{day.displayDate}</span>
                        </div>
                    )
                })}
            </div>
